...
col <- as.character(1:142)

temp <- tempfile()
download.file("https://www.sam.gov/SAM/extractfiledownload?role=WW&version=SAMV2&filename=SAM_PUBLIC_MONTHLY_V2_20201206.ZIP",temp)
data <- read.delim(unz(temp, "SAM_PUBLIC_MONTHLY_V2_20201206.dat"), quote="")
unlink(temp)

row1 <- data[1,]

# metadata https://www.sam.gov/SAM/transcript/PUBLIC_V2_EXTRACT_REFERENCES.ZIP
 metadata <- SAM_Master_Extract_Mapping_v5_8_Public_File_V2_Layout
 colnames(metadata) <- c("col_name","col_num")
...
# mine whole item-card data from all pages; separate into 500 pages at the time so code runs faster
